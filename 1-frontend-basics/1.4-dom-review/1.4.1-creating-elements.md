...
We can use the DOM to create elements and put them on the page. We use `const` to declare `coolParagraph` because we do not expect `coolParagraph` to be reassigned a new value. See [1.2: ES6](../1.2-es6-basics.md#const-for-primitive-values-that-dont-change) to review `const`.
