...
The following example demonstrates how to create an add a DOM element to a page. We use `const` to declare `coolParagraph` because we do not expect to reassign `coolParagraph`. See [1.2: ES6](../1.2-es6-basics.md#const-for-primitive-values-that-dont-change) to review `const`.
