...
Clone the base node app: [https://github.com/rocketacademy/base-node-bootcamp](https://github.com/rocketacademy/base-node-bootcamp)

```bash
git clone https://github.com/rocketacademy/base-node-bootcamp.git sequel
```
...
Similar to the `pg` connection config we defined in [Module 3.5.2: PostgreSQL Node App](../../3-backend-application/3.5-sql-applications/3.5.2-postgresql-node-app.md#pg-npm-library), the Sequelize connection config specifies parameters for Sequelize to connect to our SQL database.
...
Sequelize replaces `init_tables.sql` from Module [3.5.7: Database Setup Files](../../3-backend-application/3.5-sql-applications/3.5.7-database-setup-files.md#init_tables-sql) with a concept called "**migrations**". Database migrations are a system to manage database schema, allowing us to specify changes to database tables and columns over the lifetime of our database.
