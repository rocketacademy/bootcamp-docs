...
const violateDbConstraint = async () => {
  try {
    ...
    if (error instanceof DatabaseError) {
      console.err('This is a database error!');
      console.err(error);
    } else {
      console.err(error);
    }
...
const violateModelValidation = async () => {
  try {
    ...
    if (error instanceof ValidationError) {
      console.err('This is a validation error!');
      console.err(error);
    } else {
      console.err(error);
    }
...
db.Category.findOne({
  ...
  .catch((error) => {
    if (error instanceof ValidationError) {
      console.err('This is a validation error!');
      console.err(error);
      console.err('The following is the first error message:');
      console.err(error.errors[0].message);
