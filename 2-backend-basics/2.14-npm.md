...
After running `npm install`, our NPM project will now list the installed package as a "dependency", i.e. a package that our app depends on to run properly. 3 files will change in our project folder.
...
NPM packages almost always depend on other NPM packages, because nobody builds apps from scratch. Typical NPM packages depend on a large number of other packages. For example, [here](https://npmgraph.js.org/?q=eslint) is the dependency graph for the ESLint NPM package that we have been using since Coding Basics.
...
1. Look for the `index.js` file in the `cows` package we installed. Are you able to understand the code?
