...
Node.js always interfaces with our computer asynchronously. This is because of the relative "distance" and "speed" of parts of our computer such as RAM or hard drive. We do not necessarily want our programs to wait for data to return from parts of our computer that are "far away" or "slower". We elaborate on this in the [Internet Latency](2.9-http.md#internet-latency) section below.

## Node.js Server Callbacks

Callbacks behave similarly regardless of frontend or backend: perform logic when a specific event happens. The `handleIncomingRequest` callback in our backend server has the same conceptual functionality as a `handleClick` callback in our frontend. For `handleIncomingRequest`, the event is a request from the client \(browser\), and for `handleClick` the event is a mouse click.
