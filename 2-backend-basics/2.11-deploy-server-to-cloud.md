...
In Coding Basics and Project 1 we deployed our apps to the Internet using GitHub Pages. This was sufficient because our apps only involved static \(i.e. non-changing\) HTML, CSS, and JS files. To persist user-generated data on the Internet across multiple users and visits, we will need a place on the Internet to store that data \(e.g. a database\), and a place on the Internet with logic for manipulating that database. We can accomplish this by deploying our own servers to the Internet, i.e. the "cloud". 

In recent years popular software services like [Firebase](https://firebase.google.com/) have emerged that enable us to manipulate and store data in the cloud by calling APIs, without deploying our own servers. While convenient, these services are less customisable than managing our own servers. Many companies continue to deploy their own servers due to specific requirements \(e.g. database language, security\), and their engineers need to know how to manage servers. At RA, we will first learn how to deploy and manage our own servers before using more convenient abstractions such as [Heroku](https://www.heroku.com/) or [Firebase](https://firebase.google.com/).
...
In this module we will rent a \(free\) AWS EC2 instance, clone our server code onto it from GitHub, and run our server for the public. We will be able to access our EC2 instance via a public IP address it provides.
...
![](../.gitbook/assets/screen-shot-2020-10-30-at-8.44.26-pm%20%281%29%20%281%29%20%281%29%20%281%29%20%281%29%20%281%29.png)
...
SSH stands for "Secure Shell" and is one of the most common and secure ways to access servers remotely. It allows us to "log in" to remote servers and manipulate them via the command line. SSH communication is encrypted.

If we haven't already, unzip and store the key pair files from our EC2 instance in `~/.ssh` on our Unix machines \(use Ubuntu if on Windows\). Change the permissions of the downloaded key file to 400. This means the owner of the key file \(i.e. you\) will be able to read and write to this file. Read more on Unix file permissions [here](https://www.tutorialspoint.com/unix/unix-file-permission.htm).

```text
chmod 400 ~/.ssh/<KEYFILE_NAME>.pem
```

Log into our EC2 instance with the SSH command. The `-i` parameter specifies the keyfile to use. `ubuntu@` specifies we wish to login to the server as the `ubuntu` user, the default user for Ubuntu machines. Replace `<EC2_PUBLIC_IP>` with our EC2 instance's public IP.

```text
ssh -i ~/.ssh/<KEYFILE_NAME>.pem ubuntu@<EC2_PUBLIC_IP>
```

Read more about how to access EC2 instances via SSH [here](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/AccessingInstancesLinux.html).
...
See [AWS docs](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-ip-addressing.html#vpc-private-ipv4-addresses) on what Private IP addresses are used for.
