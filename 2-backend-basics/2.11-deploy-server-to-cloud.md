...
2. Move your SSH key pair to your `~/.ssh` folder in your Unix filesystem. If this folder doesn't exist, create it with `mkdir`. This is the standard place to store SSH keys. For WSL users, see a past student Porter's notes [here](https://www.notion.so/Where-to-put-the-SSH-key-For-windows-1565ad330b12412bbe5036372a1b21a4).
...
1. If we haven't already, unzip and store the key pair files from our EC2 instance in `~/.ssh` on our Unix machines \(use Ubuntu if on Windows\). For WSL users, see a past student Porter's notes on this [here](https://www.notion.so/Where-to-put-the-SSH-key-For-windows-1565ad330b12412bbe5036372a1b21a4).
