...
HTTP \(Hyper Text Transfer Protocol\) is a protocol built on top of TCP/IP to transfer data from 1 computer to another over the Internet. HTTP was invented to send HTML \(Hyper Text Markup Language\); Today we use HTTP for all kinds of data.
