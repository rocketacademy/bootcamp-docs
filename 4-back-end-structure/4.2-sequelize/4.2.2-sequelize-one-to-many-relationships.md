...
```text
npx sequelize-cli migration:generate --name create-categories-items-tables
```
...
db.Category.findOne({
  ...
  .then((categoryItems) => console.log(categoryItems.map(categoryItem => categoryItem.name)))
