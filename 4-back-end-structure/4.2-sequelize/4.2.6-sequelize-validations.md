...
Create a `create.mjs` script to try to violate our DB constraint.
...
db.Category.findOne({
  ...
  .then((category) => {
    return db.Item.create({
      // We will only provide 1 argument to create.mjs, so 
      // process.argv[3] will be undefined.
      name: process.argv[3],
...
When we attempt to violate the constraint we get the following error.
...
If we want to catch this DB constraint error in our program and write error-handling logic for it, we need to import the `DatabaseError` class from Sequelize. Update `create.mjs` 
