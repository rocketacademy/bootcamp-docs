...
{% hint style="warning" %}
Object Oriented Objects are different from JavaScript objects. We would generally refer to our usage of JavaScript objects so far as "hash maps". The two words are confusingly named the same but don't bear any resemblance in the Object Oriented sense.
{% endhint %}

## Creating a Class

A class by itself holds no data and does not do anything. It is the abstract representation of data.
...
Once we have defined a class we can create an instance of the class. An instance is the "_live_" instatiation of a class, made from the mold of the class definition. It has actual data inside, not just the variable names.
...
Notice that when a new `Cat` is created the `__init__` constructor method gets called \(and the print function gets called\). The **constructor** is an Object Oriented Term that refers to a method that is called when an instance of the class is being created and is used to help setup the values inside of the instance.

#### `self` 

When we write code that is for `cat1` and `cat2` we are going to be calling the instance methods inside. How do we refer to the Cat \(i.e., `cat1` and `cat2`\) itself? The `self` keyword is used in Python to refer to the object outside of the instance method. So here, self.name refers to the name _**inside**_ the cat when we call `cat1.get_name()` - this works if we are calling the function for `cat1` or `cat2.`
...
Without OOP we write code "procedurally" or "functionally". In the following code example, each person's variables and functions are stored independently of each other, resulting in many variables to keep track of, and more context to pass into each function.
...
This class instance method accesses data inside the object, makes a calculation and returns a value.
...
a.pickup(tool2)
a.pickup(tool3)
a.show_tools() # Prints "Aly has Hammer, Screwdriver, Drill"
...
b.pickup(tool1)
b.show_tools() # Prints "Bob has Hammer, Hammer"
```
