...
In the route we can look for the `isUserLoggedIn` key and customise logic based on user auth.
...
{% hint style="warning" %}
On line 34 we put the whole user record into the request. For sanitation, we may want to remove the user's password before sharing the user record with successive middleware.
{% endhint %}
