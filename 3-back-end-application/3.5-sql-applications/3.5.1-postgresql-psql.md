...
Database servers run applications that allow programs to manipulate the data inside them. In this module we'll learn how to create, manage, and deploy database servers. Note that this is separate from managing the _data_ inside the database, which is done by the SQL language.

Part of setting up database servers is setting up database schema, i.e. what tables and columns are in the database. We'll learn more about how to design database schema in [3.4.6: SQL Schema Design](../3.4-sql-language/3.4.6-sql-schema-design.md), but for now RA will provide database schema that we will implement. Web applications typically do not manage database schema; application code depends on database schema, but the schema is not defined by the application.
