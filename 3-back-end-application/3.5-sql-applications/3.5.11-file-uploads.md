...
## Frontend Configuration to Accept File Uploads

### Include `input` of Type `file` in HTML Form
...
### Use Metadata in `request.file`

In production apps we would take the information from `request.file` and save it to the DB. Notice how the file name is a cryptographic hash, and there is no file extension. This is deliberate to ensure file names are unique.
...
## Retrieving Uploaded Files in the App
...
Add `express.static` config to allow Express to serve files from the `uploads` folder.
