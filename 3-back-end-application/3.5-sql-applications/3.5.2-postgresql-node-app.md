...
Database names should match the relevant code repo name and be in snake\_case. Table names are pluralised snake\_case. Column names are singular snake\_case. Snake case is because SQL entities are case-insensitive, and `-` is a special character in some SQL implementations, while `_` is not.

## Exercise

We'll write a command line app that mirrors the todo app we built in [2.PCE.6](../../2-back-end-basics/2.poce-post-class-exercises/2.poce.6-todo-list-json.md), except with dogs.
