...
1. The browser receives a response back from the server. This response header has the `Set-Cookie` key in it. _Inside_ the `Set-Cookie` key is a set of keys and values.
...
The defining characteristic of Incognito Mode is that the cookie environment is separated from the regular browser cookie environment \(i.e., cookies are not mixed between the two\).
