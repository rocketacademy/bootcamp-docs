...
3. Called with a smalled piece of the original data every "iteration".
...
1. Function must call itself
2. Must have a base case to say when to stop.
3. Called with a smalled piece of the original data.
...
def display_ll_recursive(node):
    ...
    print(node.value, end=" --> ")
    display_ll_recursive(node.next)
...
def length_recursive(node):
    ...
    return length_recursive(node.next) + 1
...
def length_recursive(node):
    ...
    accumulated_length = length_recursive(node.next)
...
def addRecursive(node, newnode):
    ...
    if (node == None):
        ...
        node.next = addRecursive(node.next, newnode)
    return node
