...
Database names should match the relevant code repo name and be in snake_case. Table names are pluralised snake_case. Column names are singular snake_case. Snake case is because SQL entities are case-insensitive, and `-` is a special character in some SQL implementations, while `_` is not.
