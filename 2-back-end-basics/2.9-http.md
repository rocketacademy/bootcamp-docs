...
This server is available at: [http://localhost:3004](http://localhost:3004). `localhost` is the default domain name for all servers that are running on our local machines, e.g. in our terminal programs. The port is specified from within the local server application.
