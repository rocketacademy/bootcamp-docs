...
{% hint style="warning" %}
If we used only write instead of read and then write, we would _overwrite_ everything in `data.json`. Read then write allows us to perform "append" behaviour. 
{% endhint %}
