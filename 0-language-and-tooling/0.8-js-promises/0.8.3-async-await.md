...
Async/await is a more concise syntax that in most cases can accomplish the same goals as `.then`. We can strive to use async/await in as many places as possible, resorting to `.then` when the system may not support async/await syntax or when we prefer `.then` callback syntax.
