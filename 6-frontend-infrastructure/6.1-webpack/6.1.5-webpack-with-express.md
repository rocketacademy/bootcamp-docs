...
Delete the script tag (line 21 above)
...
Add line 4 to your `webpack.config.js` file and add `new HtmlWebpackPlugin({template: './src/template.html})` to line 17. Your webpack.config.js file should end up like this.
...
Running `npm run build` will generate an `index.html` file in the `dist` folder, that contains a dynamically generated `<script>` tag, line 11 and all the content of our original `index.js` file.
...
We will split our original `webpack.config.js` into 3 files: common, dev, and prod. What is shared between the development and production files are put in `webpack.common.js`, and dev and prod-environment-specific configs are in `webpack.dev.js` and `webpack.prod.js` respectively.
...
Clone the below Webpack MVC Base repo and create a demo Heroku app based on the below instructions. The repo contains the above changes and a few additions to account for differences between dev and prod environments.
